...
module.exports.pitch = function(request) {
  ...
  if(this[__dirname] === undefined) {
    ...
    if(query.extract !== false) {
      ...
      childCompiler.plugin("normal-module-factory", function(nmf) {
          nmf.plugin("before-resolve", function(result, callback) {
            if(query.filename){
              result.request = '!!' + result.request.split('!').slice(-2).join('!').replace(/(.*)\/([^\.\/]+)\.(.*)$/, function($0, $1, $2, $3){
                return $1 + '/' + query.filename + '.' + $3;
              });
              if(result.dependency){
                result.dependency.request = result.dependency.userRequest = result.request;
              }
            }
